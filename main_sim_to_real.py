...
Pipeline:
 ...
 2. Evaluate on the same three historical windows used in :
...
def train(policy, epochs, batch, lam=0.5):
 """Mean-penalised CVaR, lambda=0.5 chosen from Pareto sweep."""
...
def main():
 ...
 # -- Historical evaluation -- same methodology as -- #
 periods = {
 ...
 # policy for comparison (vanilla Heston + DR, no jumps, narrow range)
 ...
 r["DeepHedge (Heston DR, )"] = stats(pnl_old)
 r["pnl"] = dict(bs=pnl_bs, un=pnl_un, new=pnl_new, old=pnl_old,
 n_win=len(S_w))
 all_res[label] = r

 for k in ["Unhedged", "BS", "DeepHedge (Heston DR, )",
 "DeepHedge (new)"]:
 ...
 methods = ["Unhedged", "BS", "DeepHedge (Heston DR, )", "DeepHedge (new)"]
 colours = {"Unhedged": "#9E9E9E", "BS": "#2196F3",
 "DeepHedge (Heston DR, )": "#FFC107",
 ...
 f.write("-- Sim-to-real training and evaluation\n")
 f.write("=" * 60 + "\n")
