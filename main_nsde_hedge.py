...
HYPOTHESIS UNDER TEST (from diagnosis)
------------------------------------------------
showed that extending training to include 2020 COVID only closed
the COVID gap by ~23%, not the expected ~100% if coverage were the sole
issue. concluded the residual failure was driven by the lagging
EWMA proxy.
...
3. Train VIXGatedPolicy (theta=3.0, same as / Step 1) on
...
def main():
 ...
 print(" lagging-proxy hypothesis holds: generator alone")
