...
--------------------------
: Train the deep hedger directly on resampled real SPY returns
(stationary block bootstrap), no simulated paths. The goal is to
convert the sim-to-real negative into a second positive result
...
Design:
 ...
 - Eval : same 2008/2020/2017 SPY windows as , plus
...
# --- Evaluation on historical windows (reuse pipeline) ----------- #
...
def write_metrics(res, history, path="results/backtest_training_metrics.txt"):
 ...
 f.write("-- Backtest-trained deep hedger vs BS delta\n")
 f.write("=" * 72 + "\n\n")
...
def plot_cvar_bars(res, path="results/backtest_training_cvar.png"):
 ...
 ax.set_title(": backtest-trained deep hedger vs BS delta on SPY windows")
